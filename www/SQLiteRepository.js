(function() {
  var DB_STATE_INIT, DB_STATE_OPEN, READ_ONLY_REGEX, SQLiteFactory, SQLiteRepository, SQLitePluginTransaction, argsArray, dblocations, newSQLError, nextTick, root, txLocks;
  ...
  SQLiteRepository = function(openargs, openSuccess, openError) {
    var dbname;
    if (!(openargs && openargs['name'])) {
      throw newSQLError("Cannot create a SQLiteRepository db instance without a db name");
    }
  ...
  SQLiteRepository.prototype.databaseFeatures = {
    isSQLitePluginDatabase: true
  };

  SQLiteRepository.prototype.openDBs = {};

  SQLiteRepository.prototype.addTransaction = function(t) {
  ...
  SQLiteRepository.prototype.transaction = function(fn, error, success) {
  ...
  SQLiteRepository.prototype.readTransaction = function(fn, error, success) {
    if (!this.openDBs[this.dbname]) {
      error(newSQLError('database not open'));
      return;
    }
  ...
  SQLiteRepository.prototype.startNextTransaction = function() {
    var self;
    self = this;
  ...
  SQLiteRepository.prototype.abortAllPendingTransactions = function() {
  ...
  SQLiteRepository.prototype.open = function(success, error) {
    ...
    if (this.dbname in this.openDBs) {
      ...
      cordova.exec(opensuccesscb, openerrorcb, "SQLiteRepository", "open", [this.openargs]);
    }
  };

  SQLiteRepository.prototype.close = function(success, error) {
    if (this.dbname in this.openDBs) {
      ...
      cordova.exec(success, error, "SQLiteRepository", "close", [
        {
          path: this.dbname
        }
      ]);
  ...
  SQLiteRepository.prototype.executeSql = function(statement, params, success, error) {
  ...
  SQLitePluginTransaction.prototype.run = function() {
    ...
    cordova.exec(mycb, null, "SQLiteRepository", "backgroundExecuteSqlBatch", [
      {
        dbargs: {
  ...
  SQLiteFactory = {
    ...
    opendb: argsArray(function(args) {
      ...
      return new SQLiteRepository(openargs, okcb, errorcb);
    }),
    deleteDb: function(first, success, error) {
      ...
      delete SQLiteRepository.prototype.openDBs[args.path];
      return cordova.exec(success, error, "SQLiteRepository", "delete", [args]);
    }
  };

  root.SQLiteRepository = {
    sqliteFeatures: {
      isSQLitePlugin: true
    },
